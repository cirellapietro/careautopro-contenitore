  ...
  useEffect(() => {
    ...
    const fetchAllData = async () => {
      ...
      try {
        ...
        const fetchedVehicleTypes = vtSnap.docs.map(doc => ({ id: doc.id, ...doc.data() } as VehicleType));
              ...
              <TableBody>
                {allChecks.length > 0 ? allChecks.map(check => (
                  <TableRow key={check.id}>
                    <TableCell className="font-medium uppercase text-xs">{check.description}</TableCell>
                    <TableCell><Badge variant="secondary" className="uppercase text-[10px]">{check.vehicleTypeName}</Badge></TableCell>
                    <TableCell className="text-xs">{check.intervalMileage ? `${check.intervalMileage.toLocaleString('it-IT')} km` : 'N/A'}</TableCell>
                    <TableCell className="text-xs">{check.intervalTime ? `${check.intervalTime} mesi` : 'N/A'}</TableCell>
        ...
        <DialogContent>
            ...
            <div className="py-4">
                <Select onValueChange={setSelectedVehicleTypeForNewCheck} value={selectedVehicleTypeForNewCheck}>
                    ...
                    <SelectContent>
                        {vehicleTypes.map(vt => <SelectItem key={vt.id} value={vt.id} className="uppercase text-xs font-bold">{vt.name}</SelectItem>)}
                    </SelectContent>
                </Select>
            </div>
            <DialogFooter>
