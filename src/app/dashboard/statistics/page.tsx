...
export default function StatisticsPage() {
    ...
    if (loading) {
        return (
            <div className="flex h-[60vh] w-full flex-col items-center justify-center">
                ...
                <p className="mt-4 text-muted-foreground uppercase text-[10px] font-bold">Caricamento statistiche...</p>
        ...
        return (
             <div className="space-y-6">
                <div className="flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
                     <div>
                        <h1 className="font-headline text-3xl font-bold uppercase">Statistiche</h1>
                        <p className="text-muted-foreground uppercase text-xs">Panoramica del tuo utilizzo e dei tuoi veicoli.</p>
                 ...
                 <Card className="border-destructive">
                    <CardHeader>
                        <CardTitle className="flex items-center gap-2 text-destructive uppercase">
                    ...
                    <CardContent>
                        <p className="uppercase text-xs font-bold">{error}</p>
                        <p>{error}</p>
                    </CardContent>
                </Card>
            </div>
    ...
    return (
        <div className="space-y-6">
                ...
                <div className="flex items-center gap-2">
                    <div className="w-full md:w-64">
                        <Select onValueChange={setSelectedVehicleId} defaultValue="all">
                          ...
                          <SelectContent>
                            <SelectItem value="all" className="uppercase text-xs font-bold">Tutti i veicoli</SelectItem>
                            {vehicles.map((vehicle) => (
                                <SelectItem key={vehicle.id} value={vehicle.id} className="uppercase text-xs font-bold">
            ...
            <Card>
                <CardHeader>
                    <CardTitle className="font-headline uppercase">Ultime Sessioni di Guida</CardTitle>
                    <CardDescription className="uppercase text-[10px]">Dettaglio degli ultimi viaggi effettuati.</CardDescription>
                        ...
                        <TableBody>
                            {drivingSessions.length > 0 ? drivingSessions.map(session => (
                                <TableRow key={session.id}>
                                    <TableCell>
                                        <Badge variant="outline" className="uppercase text-[10px]">{vehicleMap.get(session.vehicleId) || 'Sconosciuto'}</Badge>
                                    </TableCell>
                                    <TableCell className="text-xs uppercase font-bold">{session.startTime ? new Date(session.startTime).toLocaleString('it-IT') : 'N/D'}</TableCell>
                                    <TableCell className="text-xs font-black">{session.distance.toFixed(1)} km</TableCell>
                                    <TableCell className="text-xs font-black">{session.duration} min</TableCell>
                                </TableRow>
                            )) : (
                                <TableRow>
                                    <TableCell colSpan={4} className="text-center py-8 uppercase text-xs text-muted-foreground">Nessuna sessione di guida trovata.</TableCell>
